...
//  GPSStyle
//
//  Created by Алексей Шевченко on 29.03.2023.
//
...
extension UIImage {
    func resize(size: CGSize? = nil, insets: UIEdgeInsets = .zero, fill: UIColor = .white) -> UIImage {
        ...
        guard let unwrapImage = newImage else { return UIImage() }
          return unwrapImage
        }

    func removeBackgroudIfPosible(width: Int, height: Int) -> UIImage? {
        let resizedImage: UIImage = resize(size: .init(width: width, height: height))
